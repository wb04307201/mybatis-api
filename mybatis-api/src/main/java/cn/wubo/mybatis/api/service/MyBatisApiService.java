package cn.wubo.mybatis.api.service;

import cn.wubo.mybatis.api.Constant;
import cn.wubo.mybatis.api.excption.MyBatisApiException;
...
public class MyBatisApiService {
    ...
    public MyBatisApiService(String idName, IDService<?> idService, IMappingService mappingService, IResultService<?> resultService,MyBatisApiMapper mapper) {
        this.idName = idName;
    ...
    public Object insertOrUpdateParse(String tableName, String jsonStr) {
        return doFunction(tableName, jsonStr, (tn, row) -> {
            Object id;
            if (!row.containsKey(idName) || "".equals(row.get(idName))) {
                id = idService.generalID();
                row.put(idName, id);
                mapper.insert(tableName, row);
            } else {
                id = String.valueOf(row.get(idName));
                row.remove(idName);
                ...
                if (mapper.update(tableName, row) != 1)
                    throw new MyBatisApiException(String.format("%s value %s not unique!", idName, id));
            }
            Map<String, Object> query = new HashMap<>();
            query.put("key", idName);
            query.put(VALUE, id);
